    ...
    try {
      for (var habit in _habits) {
        ...
        if (snapshot.docs.isEmpty) {
          print("No se encontraron registros diarios para el hábito ${habit.id}.");
        } else {
          _dailyRecords.addAll(snapshot.docs.map((doc) => DailyRecord.fromMap(doc.data(), doc.id)).toList());
          print("Registros diarios obtenidos para el hábito ${habit.id}: ${_dailyRecords.length}");
        }
      }
      notifyListeners();
    } catch (e) {
  ...
  Future<void> updateDailyRecord(DailyRecord dailyRecord) async {
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .collection('dailyRecords')
          .doc(dailyRecord.id)
          .update(dailyRecord.toMap());

      final index = _dailyRecords.indexWhere((record) => record.id == dailyRecord.id);
      if (index != -1) {
        _dailyRecords[index] = dailyRecord;
        notifyListeners();
      }
    } catch (e) {
